...
(** A dynamic, multi-symbol market maker on the bot-runtime interface.
    ...
    as it gets filled. Per-symbol state -- inventory, an adaptive
    half-spread, and the set of resting orders -- is tracked internally and
    updated from the events the bot receives. When the observed market spread
    blows out past [max_spread_cents] (e.g. a whale has swept a side of the
    book), the maker stands aside rather than quoting into the dislocation. *)
module Config : sig
  type t
