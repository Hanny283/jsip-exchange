...
module Config = struct
  type t =
    ...
    ; size_per_level : int
    ; num_levels : int
    ; mutable inventory_skew_cents_per_share : int
    }
  [@@deriving sexp_of]
end
...
let run (config : Config.t) conn =
  ...
  in
  let post () =
    let inventory = Hashtbl.find inventory_counter config.symbol in
    match inventory with
    | Some inventory ->
      let skew = inventory * config.inventory_skew_cents_per_share in
      Deferred.List.iter
        ~how:`Parallel
        (List.init config.num_levels ~f:Fn.id)
        ~f:(fun level ->
          let offset = config.half_spread_cents + level + skew in
          let%bind () =
            submit
              ({ symbol = config.symbol
               ; participant = config.participant
